...
class fbr:
 ...
 def _cadow1(self):
     print
     self._id= raw_input(red+'[🔧]'+green+'Enter Target Id:    '+C+p+' ')
     my = ("https://m.facebook.com/"+self._id)
     url = my
     self.br.open(url)
     self.r=re.findall('<title>(.*?)</title>',self.br.response().read())
     if len(self.r) !=0:
        self._dray= raw_input(red+'[*]'+green+'Do You Want To Report: :    '+C+p+self.r[0]+' [y/yes]'+'')
        return self._lady()
 ...
 def _indo(self):
     ...
     if self._id in self.uhoh:
         ...
         print
         time.sleep(1)
         time.sleep(red+'Error While Reporting the Id')
         time.sleep(1)
         return self._cadow1()
     else:
          return self._cadow2()
